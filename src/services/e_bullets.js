...
class BulletBuilder {
  ...
  getDirection = (enemy) => {
    const [ABx, ABy] = MyVector.getVector(this.x.value, this.y.value, enemy.x, enemy.y);
  ...
  move = () => {
    ...
    this.collision = MyVector.isCollisionFast(this.x.value, this.y.value, this.r, enemy.x, enemy.y, enemy.r);
  };
  ...
  discardEnemy = (enemy) => {
    ...
    const bulletTowerDistance = MyVector.getVectorLength(this.x.value, this.y.value, Tower.x, Tower.y);
...
function filterTargetsInRadius(bullet, radius) {
  return Enemies.enemies.value.filter((enemy) => {
    ...
    return MyVector.getVectorLength(bullet.x.value, bullet.y.value, enemy.x, enemy.y) <= radius;
  });
}
