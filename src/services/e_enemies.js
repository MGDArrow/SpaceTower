...
import Mines from '@/services/eu_mines';
...
class BaseEnemy {
  ...
  getDistanceToTower() {
    this.distance = MyVector.getVectorLength(this.x, this.y, Tower.x, Tower.y) - this.r - Tower.r;
  }
...
class NearEnemy extends BaseEnemy {
  ...
  getMoveDirection() {
    [this.coefX, this.coefY] = MyVector.getVectorNormalize(this.x, this.y, Tower.x, Tower.y);
  }

  getDistanceToWall() {
    this.distance_wall = MyVector.getVectorLength(this.x, this.y, Wall.x, Wall.y) - this.r - Wall.r;
  }
  ...
  move() {
    ...
    if (isWall) {
      [ABx, ABy] = MyVector.getVector(newX, newY, Wall.x, Wall.y);
      isCollision = MyVector.isCollision(ABx, ABy, this.r, Wall.r);
      this.target = 'Wall';
    } else {
      [ABx, ABy] = MyVector.getVector(newX, newY, Tower.x, Tower.y);
      isCollision = MyVector.isCollision(ABx, ABy, this.r, Tower.r);
      this.target = 'Tower';
    }
  ...
  collisionWithTower(newX, newY, ABx, ABy) {
    const ABd = MyVector.getLength(ABx, ABy);
    const [Cx, Cy] = MyVector.getNormalize(ABx, ABy, ABd);
    const deepCollision = this.r + Tower.r - ABd;
    this.x = newX - Cx * deepCollision;
    this.y = newY - Cy * deepCollision;
    this.rotate = MyRotate.rotatePrecise(ABx, ABy, ABd);
...
class FarEnemy extends BaseEnemy {
  ...
  getMoveDirection() {
    const [ETx, ETy] = MyVector.getVector(this.x, this.y, Tower.x, Tower.y);
    const distance = MyVector.getLength(ETx, ETy);
    [this.coefX, this.coefY] = MyVector.getNormalize(ETx, ETy, distance);
    this.distance = distance - this.r - Tower.r;
    this.rotate = MyRotate.rotate(ETx, ETy, distance);
  }
}
...
class Enemies {
  ...
  enemiesCollision(enemy) {
    ...
    quadEnemies.forEach((second) => {
      ...
      const [ABx, ABy] = MyVector.getVector(enemy.x, enemy.y, second.x, second.y);
      const isCollision = MyVector.isCollision(ABx, ABy, enemy.r, second.r);
